...
export default function OrderConfirmation({
  ...
  const messageForGuest =<Typography style={{margin : 50}}>Order successfully created.we have registered you so that you can track your order.
     You can change your profile details in the my profile tab</Typography>
     const messageForUser =<Typography style={{margin : 50}}>Order successfully created.You can track it in my Orders tab.</Typography>
 ...
 async function сreateOrder() {
        if(checked===true && emailErrorLabel==="")
        console.log("email")
    ...
    <>
        <Paper className={classes.paper}>
          ...
          >
            ...
            {isLogged===false ? <Grid item xs={12}>
              <TextField
            ...
            <Grid item xs={12}>
              <FormControlLabel
                control={<Checkbox onChange={(e) => setChecked(e.target.checked)} checked={checked} name="checkedB" color="primary" />}
                label="I agree with the information I entered"
              />
            </Grid>
            <Button variant="contained" onClick={() => сreateOrder()} color="primary">
              Order
            </Button>
          </Grid>
        </Paper>
        ...
        handleClose={handleCloseMessageDialog}
         form={useSelector((state) => state.email)===undefined ? messageForGuest : messageForUser}></BaseDialog>
        </>
  );
}
